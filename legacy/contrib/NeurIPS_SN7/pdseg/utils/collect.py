...
class SegConfig(dict):
    ...
    def check_and_infer(self):
        ...
        if self.DATASET.INPUT_IMAGE_NUM not in [1, 2]:
            raise ValueError("DATASET.INPUT_IMAGE_NUM should be 1 or 2")

        if self.TEST.TEST_AUG:
            if len(self.TEST.TEST_AUG_FLIP_OPS) + len(
                    self.TEST.TEST_AUG_ROTATE_OPS) == 0:
                raise ValueError('Must define one ops at least for test aug.')
            for ops in self.TEST.TEST_AUG_FLIP_OPS:
                if ops[0] not in ['h', 'v', 'm', 'b']:
                    raise ValueError(
                        'Error flip ops type. '
                        'Only horizontal, vertical, main_diagonal, back_diagonal supported.'
                    )
            for ops in self.TEST.TEST_AUG_ROTATE_OPS:
                if ops not in [90, 180, 270]:
                    raise ValueError(
                        'Error rotation angle. Only 90, 180, 270 supported.')
